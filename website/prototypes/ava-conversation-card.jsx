...
function Wave({ playing, progress, width = 420, height = 52 }) {
  ...
  const draw = useCallback(() => {
    ...
    const pulsePos = playing
      ? (progress * 1.4 - 0.2) // leads the speech slightly
      : (time * 0.12) % 1.6 - 0.3;
    const pulseWidth = 0.18;

    // ─── Draw waves — smooth, organic, fewer layers ─────────────────
    const layers = [
    ...
    layers.forEach(l => {
      ...
      for (let x = 0; x <= w; x += 3) {
        ...
        // Smooth composite sine — fewer harmonics, more organic
        const y = cy +
    ...
    // ─── Lightning pulse — radial energy sweep ──────────────────────
