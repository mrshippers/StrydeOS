...
function MonolithMark({ size = 44 }: { size?: number }) {
...
function Spark({ data, color, w = 56, h = 18 }: { data: number[]; color: string; w?: number; h?: number }) {
...
function ModuleCard({ name, color, stat, delay }: { name: string; color: string; stat: string; delay: number }) {
...
function KPI({ label, value, unit, delta, dir, status, sparkData, delay }: {
  label: string; value: string; unit?: string; delta?: string; dir?: "up" | "down";
  status: "green" | "red" | "grey"; sparkData?: number[]; delay: number;
}) {
  const [v, setV] = useState(false);
  useEffect(() => { const t = setTimeout(() => setV(true), delay); return () => clearTimeout(t); }, [delay]);
      ...
      <div className="ds-card" style={{ fontFamily: "'Outfit', sans-serif", width: "100%", maxWidth: 400, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 8, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "rgba(255,255,255,0.25)", marginBottom: 4 }}>Spires Physiotherapy &middot; London</div>
