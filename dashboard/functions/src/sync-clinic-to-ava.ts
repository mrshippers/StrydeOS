...
// Update both files together. The functions package cannot import from src/.
const AVA_CORE_PROMPT_TEMPLATE = `
[1 — IDENTITY]
...
function buildAvaCorePrompt(vars: {
  clinic_name: string;
  clinic_email: string;
  clinic_phone: string;
  hours: string;
  ...
  return AVA_CORE_PROMPT_TEMPLATE
    ...
    .replaceAll("{{pms_name}}", vars.pms_name);
}
...
async function syncClinicToAvaCore(clinicId: string, apiKey: string): Promise<SyncDiff> {
  ...
  const systemPrompt = buildAvaCorePrompt({
