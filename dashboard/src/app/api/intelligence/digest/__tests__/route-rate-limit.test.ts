/**
 * P0-15: Rate limit tests for GET /api/intelligence/digest
 *
 * Covers:
 *   - User over budget is rejected 429 (failClosed = true)
 *   - Cron is exempt from rate limiting
 *
 * Run: npx vitest run src/app/api/intelligence/digest/__tests__/route-rate-limit.test.ts
 */
...
// ── firebase-admin + notify-owner mocks ───────────────────────────────────────
...
vi.mock("@/lib/intelligence/notify-owner", () => ({
  sendWeeklyDigest: vi.fn().mockResolvedValue({ sent: true }),
}));

// ── helpers ───────────────────────────────────────────────────────────────────

function makeRequest() {
  return new NextRequest("http://localhost/api/intelligence/digest", { method: "GET" });
}

// ── tests ─────────────────────────────────────────────────────────────────────

describe("GET /api/intelligence/digest - P0-15 rate limiting", () => {
  beforeEach(() => {
    vi.clearAllMocks();
